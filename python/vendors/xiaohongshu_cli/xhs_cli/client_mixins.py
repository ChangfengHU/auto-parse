...
from .html_parser import extract_note_from_html, extract_user_profile_from_state
...
class ReadingEndpointsMixin:
    ...
    def _fetch_user_html(self, user_id: str) -> str:
    ...
    def get_user_info(self, user_id: str) -> dict[str, Any]:
        ...
        except (XhsApiError, NeedVerifyError) as exc:
            logger.info("get_user_info API failed (%s), falling back to HTML", exc)
            html = self._fetch_user_html(user_id)
            from .html_parser import parse_initial_state

            state = parse_initial_state(html)
            return extract_user_profile_from_state(state, user_id)

    def get_user_notes_from_html(self, user_id: str) -> dict[str, Any]:
        """Alternative to get_user_notes using SSR HTML parsing."""
        html = self._fetch_user_html(user_id)
        from .html_parser import parse_initial_state, extract_user_notes_from_state
        state = parse_initial_state(html)
        return extract_user_notes_from_state(state)
    ...
    def get_user_notes(self, user_id: str, cursor: str = "") -> dict[str, Any]:
        try:
            ...
            # Fallback to HTML only for the first page and if it's a signature/auth error
            if not cursor:
                logger.info("get_user_notes API failed (%s), falling back to HTML", exc)
                return self.get_user_notes_from_html(user_id)
            raise
