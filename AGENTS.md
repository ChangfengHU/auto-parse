...
- 机主最新纠偏：DOUYIN-PUBLISH 以用户已有的 `douyin-publish`（`/workflows/douyin-publish`）为基础，先读完整 Supabase 配置，保留原节点结构。原件冻结不修改；确需改节点，机主已授权通过正式 copy 接口复制后仅改副本，不得自行重建另一套流程替代。每次验证前后比对原件未变化。当前使用 84 已登录 `creator.douyin.com` 的 browser-2:9223；95 原浏览器方案仅为历史可选能力，不是当前执行路线。网页登录与创作者后台登录不能混为一谈。
