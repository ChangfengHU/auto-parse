...
# 用法：bash scripts/publish.sh "<ossUrl>" "<title>" ["<description>"] ["tag1,tag2"] ["dy_xxx登录凭证"]
#
# 输出格式（OpenClaw 解析用）：
#   每行日志原样打印
...
API="${BASE_URL}/api/publish"
STATUS_API="${BASE_URL}/api/publish/status"

if [[ -z "$OSS_URL" || -z "$TITLE" ]]; then
  echo "[ERROR] 用法: bash scripts/publish.sh \"<ossUrl>\" \"<title>\" \"<description>\" \"tag1,tag2\" \"dy_xxx凭证\""
  exit 1
fi
